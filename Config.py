...
class Config(object):
    def __init__(self, config_file_name):
        ...
        target = self.cf.get('douban', 'douban_url')
        str1 = target.replace(']','').replace('[','')
        self.douban_url = str1.replace('"','').split(",")
        target = self.cf.get('douban', 'douban_url_name')
        str1 = target.replace(']','').replace('[','')
        self.douban_url_name = str1
